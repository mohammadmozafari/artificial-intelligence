...
COLORING_COLORS = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]
DX = [-1, +1, 0, 0]
DY = [0, 0, -1, +1]
...
class MapColoringProblem:
    def __init__(self, map_image_path):
        print(f'loading map {map_image_path} ...')
        self.IMAGE_PATH = map_image_path
        self.image = cv2.imread(map_image_path, cv2.IMREAD_COLOR)
        if self.image is None:
            raise FileNotFoundError
        self.height = len(self.image)
        self.width = len(self.image[0])
        if self.width > MAXIMUM_IMAGE_WIDTH or self.height > MAXIMUM_IMAGE_HEIGHT:
            print("Error: please specify an image with smaller dimensions.", file=sys.stderr)
            exit(0)
        self.total_area = self.width * self.height
        self.mark = [[NOT_MARKED for _ in range(self.width)] for _ in range(self.height)]
        self.nodes = []
        self.regions = [[] for _ in range(MAXIMUM_NUMBER_OF_REGIONS)]
        self.regions_border = [[] for _ in range(MAXIMUM_NUMBER_OF_REGIONS)]
    ...
    def random_initial_state(self):
        for n in self.nodes:
            n.color = randint(0, len(COLORING_COLORS) - 1)
    ...
    def fitness(self, chromosome):
        ...
        for n in self.nodes:
            for adj_node in n.adj:
                if chromosome[self.nodes[adj_node].id] != chromosome[n.id]:
                    delta += 1
        return round(delta / m, 6)

    @staticmethod
    def mutate(chromosome):
        i = randint(0, len(chromosome) - 1)
        chromosome[i] += randint(1, len(COLORING_COLORS) - 1)
        chromosome[i] %= len(COLORING_COLORS)

    @staticmethod
    def random_successor(state):
        state = state.copy()
        i = randint(0, len(state) - 1)
        state[i] += randint(1, len(COLORING_COLORS) - 1)
        state[i] %= len(COLORING_COLORS)
        return state
    ...
    def goal_test(self, node_colors):
        return self.fitness(node_colors) == 2

    def visualize(self, node_colors, wait_time=1500, save=False):
        if wait_time < 0:
            return
        for i, n in enumerate(self.nodes):
            self.change_region_color(n, COLORING_COLORS[node_colors[i]])
        if save:
            cv2.imwrite(self.IMAGE_PATH.replace('.jpg', '_colored.jpg').replace('.png', '_colored.png'), self.image)
        else:
            cv2.imshow('Map Coloring', self.image)
            cv2.waitKey(wait_time)
			
def main():
    p = MapColoringProblem('iran_2.jpg')
    for i in p.nodes:
        print(i.adj)
    input('Enter somethin')
